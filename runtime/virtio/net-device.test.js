...
function makeVq(memory, base, num) {
  ...
  const desc = base, avail = base + num * 16, used = avail + 4 + num * 2 + 2;
  return {
    desc, avail, used, num,
    ...
    setDesc(i, addr, len, write, next) {
      ...
      dv.setUint16(d + 12, (write ? VRING_DESC_F_WRITE : 0) | (next != null ? VRING_DESC_F_NEXT : 0), true);
    ...
    usedIdx() { return dv.getUint16(used + 2, true); },
    usedElem(slot) { return { id: dv.getUint32(used + 4 + slot * 8, true), len: dv.getUint32(used + 8 + slot * 8, true) }; },
  };
}
...
function mkDevice() {
  ...
  const irqs = [];
  const d = new NetDevice({ dev: 2, irq: 10, memory, raiseInterrupt: (cpu, irq) => irqs.push([cpu, irq]), sharedQueues: shared, mac: [0x52,0x54,0x00,0xcb,0x00,0x02] });
...
test("pushRx writes header+frame into an RX buffer and raises IRQ", () => {
  ...
  const dv = new DataView(memory.buffer);
  expect([dv.getUint8(0x4000 + VIRTIO_NET_HDR_LEN), dv.getUint8(0x4000 + VIRTIO_NET_HDR_LEN + 1), dv.getUint8(0x4000 + VIRTIO_NET_HDR_LEN + 2)]).toEqual([1,2,3]);
