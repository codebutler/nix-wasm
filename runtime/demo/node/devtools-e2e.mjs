...
// cache via `nix-env -iA dev-tools`, then used to compile and run a C program.
//
// Steps:
//   1. Boot the nix system (full /nix overlay from squashfs + nix-cache).
//   2. Assert `clang` is NOT in $PATH (toolchain removed from base).
//   3. `nix-env -iA dev-tools` — substitutes the toolchain from /nix-cache.
...
// where nix-cache/ is the .#wasm-binary-cache output (has dev-tools attr in pkgs.nix).
//
...
try {
  ...
  // Step 3: install dev-tools from the binary cache
  console.log("  [installing dev-tools from nix-cache — may take a while…]");
  s.send("nix-env -iA dev-tools 2>&1; echo NIX_ENV_RC=$?\n");
  ...
  check(installOk, "nix-env -iA dev-tools substitutes from /nix-cache");
  if (!installOk) {
    console.log(
      "\n── nix-env output (tail) ──\n" +
        s
  ...
  check(clangPresent, "clang now in PATH after nix-env -iA dev-tools");
