...
// the software MMU and prove its steady-state fork()+exec() milestone.
//
...
// execs /bin/sh; the shell prints a builtin witness line. That proves busybox
// init boots as PID 1 and its fork()+exec() steady-state works under the MMU —
// the exact thing the clone-hack could not do.
//
// SCOPE: this gate asserts the init fork+exec MILESTONE, not a full interactive
// shell. A shell fork+waiting for a non-last external command still mis-rewinds
// (whole-module asyncify vs busybox's deep fork stack) — see the KNOWN
// LIMITATION in userspace/busybox-fork.nix. That is the remaining slice-1 work.
//
...
const initScript = ["exec >/dev/console 2>&1", 'echo "BBFORK: init alive pid=$$"'].join("; ");
const inittab =
...
let pass = false;
try {
  // The witness line is printed by the fork+exec'd /bin/sh child — proving
  // busybox init booted as PID 1 and its steady-state fork()+exec() works.
  const ok = await s.waitForOutput(/BBFORK: init alive pid=/, 180000).catch((e) => {
...
console.log("\n[busybox-fork-smoke] " + (pass ? "PASS (init fork+exec milestone)" : "FAIL"));
