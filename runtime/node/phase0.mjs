...
try {
  ...
  if (single.length) {
    ...
    await s.waitForOutput(new RegExp("RESULT " + esc(single[single.length - 1]) + " (PASS|FAIL)"), 20000);
  }
  ...
  for (const n of names) {
    if (new RegExp("RESULT " + esc(n) + " PASS").test(out)) console.log("  ok   " + n);
    else { const m = out.match(new RegExp("RESULT " + esc(n) + " FAIL[^\\n]*")); console.log("  FAIL " + n + (m ? "  (" + m[0] + ")" : "  (no RESULT line)")); pass = false; }
  }
  if (!pass) console.log("\n--- transcript tail ---\n" + out.slice(-1400));
} finally {
  s.kill();
