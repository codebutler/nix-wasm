...
cross.stdenv.mkDerivation {
  ...
  patches = [
    ...
    # redirect" that made every real autoconf `configure` unparseable under
    # hush. Applies zero-fuzz on top of 0001+0003+0006 above (none of those
    # touch the redirect-parsing code this edits); shared correctness fix,
    # wired here even though this build's default /bin/sh is ash's forkshell
    # (userspace/ash.nix) — busybox.nix still compiles hush.c.
  ...
  # identical passthru for the full rationale. This is the NOMMU,
  # clone-with-fn guest (ordinary programs use posix_spawn; a direct fork
  # caller cannot link without the deliberately excluded capture_stack seam);
