...
> **Task-0 spike (2026-06-20) — B3 RESOLVED POSITIVELY.** A toolchain-free WAT
> probe and a real clang `-O2` probe both prove the asyncify double-return:
> run → unwind to the host run-loop → verbatim memory copy → rewind both → a
> single `do_fork()` returns twice, resume lands exactly at the call site
> (pre-fork code not replayed, two frames deep), memories isolated, live C
> locals + shadow stack survive. See `spikes/asyncify-fork/FINDINGS.md`.
> Three actionable findings: **(A)** the parent→child memory copy MUST happen
> *after* child-instance creation (a fresh instance re-applies active data
> segments, clobbering copied `.bss`) — locks Task 4 ordering; **(B)** use a
> **dedicated `capture_stack()` import** as the unwind point, NOT the generic
> `__wasm_syscall_N` wrapper — so the syscall fast path stays asyncify-free and
> B3's "unwind through the syscall wrapper" worry is sidestepped entirely;
> **(C)** pre-fork state must be materialized in memory before capture (`-O2`
> can fold a pre-fork store past the fork point).
...
- **Asyncify cost (known).** Size/perf tax on fork-capable binaries (WASIX moved
  ...
  *Mitigation:* clone-with-fn fast path + allow-listed asyncify. *Open:* measure the
  tax on a real fork-without-exec binary; set allow-list scope empirically (the
  spike's +625 B is fixed scaffolding on a trivial probe — not representative).
